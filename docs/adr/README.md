...
- [ADR-0012: Effort ladder and PR mode for `/expert-review`](0012-effort-ladder-and-pr-mode.md) — a manual `--effort 1–5` ladder (swarm → full panel) plus a positional GitHub-PR-URL mode that folds coworker review into `/expert-review`; deprecates (not deletes) `/expert-review-coworker(-beta)` and records effort 1's deliberate non-blindness — amended: default effort (tiers 2-4) is now heuristic-derived from `effort-heuristic.yaml` rather than a flat 4
