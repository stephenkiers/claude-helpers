...
- [ADR-0012: /stack-sync — layout-routed stack sync](0012-stack-sync.md) — the layout-routed **sync** mirror of ADR-0011's layout-routed push: single-driver delegates to `gh stack sync`, per-branch walks descendants bottom-up (verified — not ordered — by `git merge-base --is-ancestor`, not cache) and rebases each via a generalized Restack-a-child block; unknown layout fails closed; per-branch force-push is gated on one confirmation plus the repo check gate
