...
if __name__ == "__main__":
    ...
    test_result(
        ...
        "outside the repo" not in expert_review_content and "decisions.yaml" not in expert_review_content,
    ...
    test_result(
        ...
        "ledger" not in expert_review_content.lower() and "DECISIONS_FILE" not in expert_review_content,
        "Found ledger or DECISIONS_FILE reference in cross-run memory section"
    )
    ...
    # 1.9: Step 13 (Cache Review Metadata) should be present after Step 13: Record was removed
    test_result(
        "Cache Review Metadata step still present (renumbered)",
        "### Step 13: Cache Review Metadata" in expert_review_content,
        "Cache Review Metadata step should still exist as Step 13"
    )
    ...
    test_result(
        ...
        "recorded-decisions" not in expert_framework_content and "DECISIONS_FILE" not in expert_framework_content,
    ...
    test_result(
        ...
        "Already settled" not in triage_content and "## Already settled" not in triage_content,
    ...
    # 3.8: "Recurring?" bullet in gut check should be removed entirely
    test_result(
        "Recurring bullet removed from gut check",
        "Recurring?" not in triage_content and "ledger.jsonl" not in triage_content,
        "Found 'Recurring?' bullet or ledger.jsonl in gut check (should be removed)"
    )
    ...
    test_result(
        ...
        "Already settled" not in claude_md_content,
    ...
    for phrase, should_not_be_in in forbidden_phrases:
        ...
        test_result(
            ...
            len(violations) == 0,
            f"Still found in: {', '.join(violations)}" if violations else ""
        )
