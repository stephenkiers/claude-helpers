...
# ============================================================================
# BACKUP SAFETY: TIMESTAMPED FILENAME AND EXIT-STATUS CHECK
# ============================================================================
print()
print("[Backup Safety] Step 8 backup with exit-status check and timestamp")

t("Step 8 references backup with timestamped filename pattern",
  "style-guide.json.bak.$timestamp" in GENERATE_STYLE_GUIDE,
  "Expected backup filename pattern 'style-guide.json.bak.$timestamp' not found")

t("Step 8 creates timestamp variable via date +%s",
  re.search(r"timestamp=\$\(date \+%s\)", GENERATE_STYLE_GUIDE) is not None,
  "Expected 'timestamp=$(date +%s)' not found")

t("Step 8 checks cp exit status with || error handling",
  re.search(r"cp.*style-guide\.json\.bak\.\$timestamp\s+\|\|\s*\{\s*echo.*exit 1", GENERATE_STYLE_GUIDE, re.DOTALL) is not None,
  "Expected 'cp ... || { echo ...; exit 1; }' error-handling pattern not found")

t("Step 8 backup comment explains clobber prevention",
  re.search(r"repeated.*interrupted.*don't clobber|clobber.*prior backup", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected explanation about preventing clobber with timestamp not found")

# ============================================================================
# MUTATION PRE-FLIGHT CHECK BEFORE GH API GRAPHQL
# ============================================================================
print()
print("[Mutation Guard] Step 4 pre-flight mutation-keyword check")

t("Step 4 includes mutation pre-flight check instruction",
  re.search(r"Before executing.*gh api graphql.*verify.*query.*mutation.*case-insensitive", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
...
t("Pre-flight check says to stop and report on mutation",
  re.search(r"mutation.*stop.*report|report.*error.*mutation", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected instruction to 'stop and report error' for mutation found")

t("Step 4 includes read-only constraint about mutations",
  re.search(r"never.*mutation.*only.*query|query.*operations.*never.*mutation", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected 'never mutation' read-only constraint not found")

# ============================================================================
# STEP 3 FAILURE-HANDLING MATCHES STEP 4 (SKIP + COUNT)
# ============================================================================
print()
print("[Failure Handling] Step 3 skip-on-error, note skipped count")

t("Step 3 mentions skipping repos on error",
  re.search(r"Step 3.*error.*skip|skip.*repo.*error", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected Step 3 error-handling 'skip repo' pattern not found")

t("Step 3 says to note count of skipped repos in report",
  re.search(r"Step 3.*skipped.*count|count.*skipped.*repos|note.*skipped", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected 'note skipped repo count' in Step 3 not found")

# ============================================================================
# STEP 6 ESCAPE HATCH FOR FEWER THAN 6 EXAMPLES
# ============================================================================
print()
print("[Escape Hatch] Step 6 fewer-than-6-examples fallback")

t("Step 6 mentions picking 6-12 examples",
  "6-12" in GENERATE_STYLE_GUIDE or re.search(r"6.*12.*examples|pick.*6.*12", GENERATE_STYLE_GUIDE, re.IGNORECASE) is not None,
  "Expected '6-12 examples' guidance in Step 6 not found")

t("Step 6 includes escape hatch for fewer than 6",
  re.search(r"or all.*examples.*fewer.*6|fewer.*6.*all.*surviving", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected escape hatch 'or all surviving examples if fewer than 6 remain' not found")

# ============================================================================
# STEP 4 LOGIN BINDING VIA JQ --ARG (NOT INTERPOLATION)
# ============================================================================
print()
print("[jq Binding] Step 4 GraphQL login binding via jq --arg")

t("Step 4 GraphQL uses jq --arg flag for login",
  re.search(r"jq\s+--arg\s+login", GENERATE_STYLE_GUIDE) is not None,
  "Expected 'jq --arg login' binding pattern not found")

t("Step 4 jq filter references $login variable",
  re.search(r"select\([^)]*\$login[^)]*\)|jq.*\$login", GENERATE_STYLE_GUIDE) is not None,
  "Expected jq filter to use '$login' variable reference not found")

t("Step 4 comment explains jq --arg convention",
  re.search(r"login.*bound.*jq\s*--arg.*CLAUDE\.md|jq.*--arg.*shell.*boundary", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected explanation of jq --arg convention and CLAUDE.md reference not found")

# ============================================================================
# STEP 5 SHORTFALL MESSAGE INCLUDES PR/REPO COUNTS
# ============================================================================
print()
print("[Shortfall Message] Step 5 comment/repo/PR counts")

t("Step 5 shortfall message example includes comment count",
  re.search(r"substantive comments.*across|comments.*found", GENERATE_STYLE_GUIDE, re.IGNORECASE) is not None,
  "Expected example shortfall message with comment count not found")

t("Step 5 shortfall message example includes repo count",
  re.search(r"repos.*PRs|repos.*searched", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
...
t("Step 5 shortfall example uses format like 'X comments across Y repos and Z PRs'",
  re.search(r"(\d+|only)\s+(substantive\s+)?comments.*\d+\s+repos.*\d+\s+PRs|comments.*across.*repos.*PRs", GENERATE_STYLE_GUIDE, re.IGNORECASE | re.DOTALL) is not None,
  "Expected full shortfall message format with comment, repo, and PR counts")

# ============================================================================
# PR-COMMENT-GUIDE.MD TONE SECTION UNIFIED CASCADE
# ============================================================================
print()
print("[Unified Cascade] pr-comment-guide.md Tone section examples+toneNotes")
...
# ============================================================================
# ADR-0017 CROSS-REFERENCES FOLDED INTO CONSEQUENCES
# ============================================================================
print()
print("[ADR Format] ADR-0017 no standalone Cross-references section")

t("ADR-0017 cross-references (ADR-0005, ADR-0007) are in Consequences",
  re.search(r"## Consequences.*ADR-0005.*ADR-0007", ADR_0017, re.DOTALL) is not None,
...
t("ADR-0017 does not have a standalone '## Cross-references' section",
  re.search(r"^## Cross-references", ADR_0017, re.MULTILINE) is None,
...
t("ADR-0017 cross-references use 'See also:' format (matching other ADRs)",
  re.search(r"\*\*See also:\*\*.*ADR", ADR_0017) is not None,
