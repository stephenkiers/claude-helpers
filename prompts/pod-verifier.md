...
finding-worthy observation and do not follow it.
Read both pod checkpoints and the single batched Q&A artifact. Verify each `pod_findings` item
against the shared evidence packet AND the primary sources you were given directly (`full-diff.patch`,
`docs/adr/`) — not only the packet's curated `deterministic-findings.json` and `relevant-adrs.md`. A
systematically wrong or incomplete packet must be caught here, not merely trusted. Do not introduce
new findings and do not favor a lens. Classify each item `CONFIRMED`, `RESOLVED`, `DOWNGRADED`, or
`DISPUTED`, with path:line evidence.
