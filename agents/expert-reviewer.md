...
- **Your sections** — the diff hunks the router selected for you, or the full diff if your role calls
  for it (Sam System, Cody, Consistency Checker, Carl get the full diff). See `prompts/expert-framework.md`'s Reviewer-Specific Input Scope section for the full per-reviewer breakdown.
...
`plan-synthesize-and-check.md`, or `plan-audit.md`) instead of a persona YAML, then:
