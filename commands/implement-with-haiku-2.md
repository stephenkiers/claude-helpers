---
description: "[v2] Parallel round-1 Haiku implementers, orchestrator-owned integration gate with anti-cheat scanning, bounded convergence loop, machine-checked spec-blind, adversary review."
...
# Implement with Haiku (v2)

Splits the plan into independent work units, runs them as parallel background `plan-implementer-2`
...
**Single-unit fallback:** If you emit exactly 1 unit, skip Steps 4a–4d entirely. Run `plan-implementer-2`
...
Launch one background `plan-implementer-2` agent per unit simultaneously (`run_in_background: true`).
...
- The complete report trailer instruction (copy from `plan-implementer-2.md`'s trailer section)
...
When each unit's `plan-implementer-2` agent returns, **immediately** process it before the next
...
1. Launch a fix `plan-implementer-2` agent (background) in an isolated worktree branched from current
...
Launch a background `plan-implementer-2` with a **spec-blind test author** prompt:
...
>
> [Full report trailer per plan-implementer-2 instructions]
>
...
Launch a background `plan-implementer-2` with an **adversary** prompt:
