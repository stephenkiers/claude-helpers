...
5. After the loop, if the user selected/kept at least one comment, write `${REVIEW_DIR}/posted-comments.md` with chosen comments as copy-pasteable blocks (one block per finding, in order chosen). If no comments were selected, skip writing the file and note that nothing was selected. Also write a structured sibling artifact `${REVIEW_DIR}/selected-comments.json` (using the **Write tool**, never shell interpolation, so comment bodies are never passed through the shell) containing only the kept findings, so a later `/leave-pr-comments` command can consume the kept set without parsing markdown. The JSON object has `pr_url` (the `PR_URL` arg), `head_sha` (the short SHA from `pr-context.md` / `HEAD_SHA`), and a `findings` array; each finding entry carries `path`, `start`, `end`, `severity` (`CRITICAL`, `HIGH`, or `MEDIUM`), `body` (the draft comment with its outer markdown fence stripped), and `permalink` (the same `https://github.com/{TARGET_REPO}/blob/{HEAD_SHA}/{path}#L{start}-L{end}` URL the walk-through already constructs). Conserve these fields from the data the walk-through already holds when presenting each finding — do not re-derive them. If no comments were kept, skip writing `selected-comments.json` too (same gate as `posted-comments.md`).
