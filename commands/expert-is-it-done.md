...
allowed-tools: Bash(gh issue view:*), Bash(gh issue edit:*), Bash(gh pr list:*), Bash(gh pr view:*), Bash(git log:*), Bash(git diff:*), Bash(grep:*), Bash(rg:*), Bash(date:*), Bash(jq:*), Read, Glob, Skill, Write, AskUserQuestion
---
...
- Read-only: no `Edit`, no write-capable `Bash`, no `gh issue edit`/`gh pr` mutation calls except
  the one explicit `gh issue edit --add-label` offered at the very end, gated behind
...
3. Else stop: "No issue number given and none found in `.claude/github-cache.json`. Usage:
   `/expert-is-it-done <issue-number>`."
...
not every contributor uses closing keywords:

```bash
PR_LIST_JSON=$(gh pr list --search "$ISSUE_NUM in:body" --state merged \
  --json number,title,url,baseRefName,headRefName,mergedAt,files 2>/dev/null || echo '[]')

PR_COUNT=$(echo "$PR_LIST_JSON" | jq 'length')
if [ "$PR_COUNT" -eq 0 ]; then
  echo "No merged PRs found referencing #$ISSUE_NUM via body search."
  echo "If PRs exist but don't mention the issue number in their body, list them manually and re-run with that context, or pass them to this command's Phase 1 by hand."
fi
```
...
**Union the touched files** across every found PR (used by Checks 1, 2, and 4):
...
can tell "not covered" from "test files misconfigured").
...
```bash
TODAY=$(date +%F)
# For each flag/date pair extracted from the registry, compare as plain ISO strings —
# lexical comparison is correct for YYYY-MM-DD.
if [[ "$FLAG_CLEANUP_DATE" < "$TODAY" ]]; then
...
```
/expert-review https://github.com/<owner>/<repo>/pull/<N> --effort 2
```
...
If the epic spans N merged PRs, this means N separate `/expert-review` invocations, each
contributing its own `final-report.md` under `~/.claude/reviews/`. List every resulting
`REVIEW_DIR` in this command's own report (Phase 2) rather than re-synthesizing their findings —
each is already a complete, standalone report.
...
Always runs (no project-specific configuration needed for the first two sub-checks):

1. **TODO/FIXME referencing the issue:** `rg -n "TODO|FIXME" $TOUCHED_FILES` filtered to lines
   also mentioning `#$ISSUE_NUM` or the issue number bare.
...
```bash
REPORT_DIR="/tmp/expert-is-it-done.issue-${ISSUE_NUM}"
mkdir -p "$REPORT_DIR"
```
...
<one-line summary: X merged PR(s) found, Y checks ran, Z configured/skipped>
...
<one of: "Close #<N> as-is — no blocking gaps found." or "Open follow-up issue(s) for the Missing
items before closing #<N>." Never both.>
```
